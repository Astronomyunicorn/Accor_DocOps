...
* Offer free hosting for public repos (and private repos with GitHub
Enterprise)
...
* *Cons:* Requires infrastructure, maintenance overhead, costs, SSL
management
* *Why not chosen:* Too much overhead for documentation site

==== Alternative B: AWS S3 {plus} CloudFront

* *Pros:* Scalable, CDN benefits, enterprise-grade
* *Cons:* Setup complexity, costs (even if low), need to manage AWS
resources
...
* *Cons:* External dependency, potential vendor lock-in, may hit free
tier limits
...
[arabic]
. *Zero Cost* — Free hosting for organization repos
. *Simple Deployment* — Push to `gh-pages` branch or use GitHub Actions
. *Automatic HTTPS* — Secure by default
. *Reliability* — GitHub’s proven infrastructure
. *Integration* — Works seamlessly with GitHub Actions CI/CD

==== Negative ❌

[arabic]
. *Public Repos Only* — Free tier limited to public repos (private repos
need Enterprise)
* *Mitigation:* Use GitHub Enterprise for private repos
. *Limited Customization* — Fewer options than self-hosted
* *Mitigation:* Sufficient for documentation needs
. *Rate Limits* — Very high limits, unlikely to hit for documentation
...
* https://github.com/actions/deploy-pages[GitHub Actions for Pages]
