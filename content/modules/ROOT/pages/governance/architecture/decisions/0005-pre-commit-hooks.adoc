...
* *Cons:* Manual setup per developer, harder to maintain, version
control issues
...
* *Cons:* Different IDEs, not all developers use same IDE, harder to
enforce
...
. *Reduced CI Load* — Fewer validation failures in CI
. *Education* — Developers learn correct format quickly
. *Consistency* — Same checks for everyone
. *Version Controlled* — Hook config is in repo, easy to update

==== Negative ❌

[arabic]
. *Setup Required* — Developers need to install pre-commit
* *Mitigation:* One-time setup, documented in onboarding
. *Commit Delay* — Validation adds ~5-10 seconds to commit
...
*Framework:* https://pre-commit.com/[pre-commit] — Python-based hook
framework
...
* `validate++_++tags.py` — Tag format and allowlist validation
* `check++_++links.py` — Internal link validation (quick check)
...
* link:./0004-ci-cd-pipeline.md[ADR-004: GitHub Actions CI/CD]
